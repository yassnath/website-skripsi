...
export default function InvoiceAddPage() {
  ...
  const validate = () => {
    const required = ["no_invoice", "nama_pelanggan", "tanggal", "status"];
    const empty = required.filter((f) => !form[f]);

    if (empty.length > 0) {
      const msg = `Please fill the required fields: ${empty.join(", ")}`;
      setErr("");
    ...
    if (!Array.isArray(form.rincian) || form.rincian.length === 0) {
      const msg = "At least one invoice detail is required.";
      setErr("");
      showPopup("danger", msg, 0);
      return false;
    }
    ...
    for (let i = 0; i < form.rincian.length; i++) {
      ...
      if (r.armada_id && (!r.armada_start_date || !r.armada_end_date)) {
        const msg = `Detail #${i + 1}: If an armada is selected, 'Departure Date' and 'Arrival Date' are required.`;
        setErr("");
        showPopup("danger", msg, 0);
        return false;
      }
  ...
  const handleSave = async () => {
    ...
    try {
      const rincianClean = (form.rincian || [])
        .map((r) => ({
      ...
      if (rincianClean.length === 0) {
        const msg = "At least one invoice detail must contain data.";
        setErr("");
        showPopup("danger", msg, 0);
        setSaving(false);
        return;
      ...
      if (saved?.id) {
        ...
        const msg = "Invoice saved, but the ID was not found.";
        setErr("");
        showPopup("danger", msg, 0);
      }
    } catch (e) {
